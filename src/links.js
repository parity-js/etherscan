...
const url = (isTestnet = false, netVersion = '0', defaultPrefix = '') => {
  return `https://${getUrlPrefix(isTestnet, netVersion, defaultPrefix)}etherscan.io`;
};

const txLink = (hash, isTestnet = false, netVersion = '0') => {
  return `${url(isTestnet, netVersion)}/tx/${hash}`;
};

const addressLink = (address, isTestnet = false, netVersion = '0') => {
  return `${url(isTestnet, netVersion)}/address/${address}`;
};

const apiLink = (query, isTestnet = false, netVersion = '0') => {
